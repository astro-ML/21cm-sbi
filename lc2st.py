...
class LC2ST:
    ...
    def _train(
        ...
        if self.num_folds > 1:
            ...
            with alive_bar(len(cv_splits[0]), title="Cross-validation", force_tty=True, refresh_secs=5) as bar:
    ...
    def train_under_null_hypothesis(
        ...
        trained_clfs_null = {}
        with alive_bar(self.num_trials_null,
            title=f"Training the classifiers under H0, permutation = {self.permutation}",
            force_tty=True, refresh_secs=5) as bar:
    ...
    def get_statistics_under_null_hypothesis(
        ...
        with alive_bar(self.num_trials_null,
                       title=f"Computing T under (H0) - permutation = {self.permutation}",
                          force_tty=True, refresh_secs=5) as bar:
            for t in range(self.num_trials_null):
                # prepare data
                if self.permutation:
...
class EnsembleClassifier(BaseEstimator):
    ...
    def fit(self, X, y):
        with alive_bar(self.num_ensemble, title="Training ensemble", 
                       force_tty=True, refresh_secs=5) as bar:
            for n in range(self.num_ensemble):
                clf = clone(self.clf)
